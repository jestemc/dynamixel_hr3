...
class DxlChain:
    ...
    def get_reg(self,id,name):
        ...
        if id not in self.motors.keys():
            raise_(DxlConfigurationException,'Motor ID %d does not exist on the chain'%(id)) 
        m=self.motors[id]
        reg=m.registers[name]
        (esize,cmd)=m.getRegisterCmd(name)
        (nid,data)=self.comm(id,cmd)
        if len(data)!=esize:
            raise_(DxlCommunicationException,'Motor ID %d did not retrieve expected register %s size %d: got %d bytes'%(id,name,esize,len(data))) 
        v=reg.fromdxl(data)
    ...
    def sync_write_pos_speed(self,ids,positions,speeds): 
        ...
        for id in ids:
            if id not in self.motors.keys():
                raise_(DxlConfigurationException,"Motor ID %d cannot be found in chain"%id)
            m=self.motors[id]
            reg_name="goal_pos"
            if reg_name not in m.registers.keys():
                raise_(DxlConfigurationException,"Synchronized write %s impossible on chain, register absent from motor ID %d"%(reg_name,id))
            r=m.registers[reg_name]
            if regpos==None:
                regpos=r
            else:
                if regpos.address!=r.address:
                    raise_(DxlConfigurationException,"Synchronized write %s impossible on chain, mismatch in register address for motor ID %d"%(reg_name,id))
                if regpos.size!=r.size:
                    raise_(DxlConfigurationException,"Synchronized write %s impossible on chain, mismatch in register size for motor ID %d"(reg_name,id))
    ...
    def set_configuration(self,conf):
        ...
        for id in conf.keys():
            sid=id
            iid=int(sid)
            if iid not in self.motors.keys():raise_(DxlConfigurationException,"Cannot find motor ID %d to be configured"%iid)
            ...
            for (name,val) in conf[sid].items():                                
                ...
                if not 'w' in reg.mode: # read only: generate error if setting is EEPROM
                    if reg.eeprom:raise_(DxlConfigurationException,"Invalid EEPROM value in motor ID %d register %s: current=%d expected=%d"%(iid,name,current,val))
                    else: pass

            # Check/Set all registers
            for (name,val) in conf[sid].items():
                if name not in motor.registers.keys():raise_(DxlConfigurationException,"Cannot configure missing register %s on motor ID %d"%(name,iid))                    
                reg=motor.registers[name]
                current=self.get_reg(iid,name)
                if current==val: continue
                # Value has to be changed
    ...
    def get_motors(self,ids=None):
        ...
        if ids==None:
            ...
            return [ids]
        raise_(Exception,"Invalid type for motor id: %s"%str(ids))
